			...
			{#if posts && posts.length}
				<div id="wall-posts">
					{#if wall.pinned_post_index !== null}
						<Post api={api} dict={dict} actions={actions} my={my} profile={profile} wall={wall}
							post={posts.find(p => p.index === wall.pinned_post_index)}/>
					{/if}
					{#each posts.filter(p => p.index !== wall.pinned_post_index) as post}
						<Post api={api} dict={dict} actions={actions} my={my} profile={profile} wall={wall} post={post}/>
					{/each}
				</div>
			{:else}
