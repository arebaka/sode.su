<script type="text/javascript">
	...
	import { api, dict, me, my, toasts, param, confirmation } from "./store";
	import { hideToast } from "./tools";
...
<Router>
	{#if $api && $dict}
		...
		{#if $confirmation}
			<div id="confirmation-box" on:click={() => {$confirmation = null}}>
				<div id="confirmation" on:click|stopPropagation>
					<h1 id="confirmation-headline">{$confirmation.dict.headline}</h1>
					<p id="confirmation-text">{$confirmation.dict.text}</p>
					<button id="confirmation-yes-button"
							on:click={$confirmation.onYes}
							on:click={() => {$confirmation = null}}>
						{$confirmation.dict.yes}
					</button>
					<button id="confirmation-no-button"
							on:click={$confirmation.onNo}
							on:click={() => {$confirmation = null}}>
						{$confirmation.dict.no}
					</button>
				</div>
			</div>
		{/if}
		...
		<main id="container">
			...
			<Route path="/:descriptor" let:params>
				<Entity descriptor={params.descriptor}/>
			</Route>
			<Route path="*">
				<Error code={404}/>
			</Route>
